...
boundary exists, and it is recorded in the shared ledger as
INC-20260729-0854-shared, whose structural cause is stated in one line: measure
the carrier, never the mention.
...
live in per-repo configuration, and once they cannot, the kit is the only place
left. That is the correct home anyway: the identifiers are the same author's in
every workspace, so a token removed in one repository and kept in the other is
exactly the drift this kit exists to stop, and it has already happened once.

CONSEQUENCE, stated rather than discovered: widening the token set is a KIT
promotion, not a local edit. A repository that notices a new identifier cannot
add it alone. That cost is deliberate; the alternative reintroduces the drift.

What a repository's config carries is what genuinely differs: which of its files
carry an identifier BY DECISION (authorship), and which paths its floors name.
No repository's list appears in this body.
...
USAGE = (
    "usage: check_shipped_surface.py --config <path> [--tree <root>] [--dist <dir>]"
)
...
#: instead of being exempted by its name alone. This is kit-level because the
...
def _coverage(
    ...
    for entry in floor:
        if entry not in result.scanned:
            where = (
                "it was exempt" if entry in result.exempt
                else "it was undecodable" if entry in result.undecodable
                else "it is absent"
            )
            violations.append(
...
def _git(root: Path, *args: str) -> tuple[int, str]:
    ...
    try:
            ...
            ["git", *args], capture_output=True, cwd=str(root), check=False
        )
    except OSError as exc:
        raise ConfigError(
...
def scan_tree(root: Path, config: Config) -> tuple[list[str], list[str]]:
    ...
    report.append(
        f"  {result.examined_chars} character(s) examined of "
        f"{result.decoded_chars} decoded"
    )
...
def scan_dist(dist: Path, config: Config) -> tuple[list[str], list[str]]:
    ...
    for label, archive, names, payload, floor in (
        ...
        if not names:
            violations.append(
                f"the {label} holds no readable member, so scanning it "
                f"certifies nothing"
            )
...
def check(
    config: Config, tree: Path | None, dist: Path | None
...
def main(argv: list[str]) -> int:
    ...
    if violations:
        ...
        return 1
    print(
        "\nno forbidden identifier found, within what the VERIFIED line above "
        "actually read"
    )
    return 0
