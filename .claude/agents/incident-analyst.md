...
structural cause on its first occurrence.** ITACA and its sister
pyflightstream both ship to PyPI, so "we will watch out for it" is not
an outcome.
...
6. **Cross-repository impact**: the two libraries share a workspace
   architecture and a review process (DD-23), so assume a shared
   structural cause until you have checked the sister repository and
   found otherwise. Say which one you checked and how.
...
- a shared mutable resource with concurrent writers and no uniqueness
  check;
- a guard whose assertion is discharged by having nothing to assert
  about, so an empty scope reads as a clean scope;
...
author: that pressure is exactly what the protocol exists to resist.
