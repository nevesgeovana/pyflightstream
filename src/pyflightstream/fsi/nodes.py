...
to tip. Node coordinates live in the rotating blade frame of
:mod:`pyflightstream.fsi.config` (X chordwise toward the leading edge,
Y normal, Z spanwise), the frame the dry-run node import used (span
along the third CSV column, ``tests/fixtures/fsi/structural_nodes.csv``).
Every blade shares the same local coordinates in its own rotating
frame, so one node file serves all blades: the import is repeated once
per blade frame, in blade order, and the flat ``FSIDisp.txt`` rows
follow that same blade-major order.
...
def generate_node_layout(cfg: FsiConfig) -> NodeOrderingMap:
    ...
    of the local chord (DLV-007 Section 4.4).
...
def node_positions(node_map: NodeOrderingMap) -> np.ndarray:
    ...
    -------
    numpy.ndarray
        Shape ``(nodes_per_blade, 3)``: (X chordwise, Y normal,
        Z spanwise) [m] in the rotating blade frame, stations root to
        tip, roles in :data:`~pyflightstream.fsi.kinematics.NODE_ROLES`
        order.
    """
    rows = []
    for i, radius in enumerate(node_map.station_radii_m):
        ...
        rows.append((e_c, e_n, radius))
        rows.append((e_c + node_map.le_offset_m[i], e_n, radius))
        rows.append((e_c + node_map.te_offset_m[i], e_n, radius))
...
def flatten_blade_translations(
    ...
    numpy.ndarray
        ...
        role, matching the import order (FSI-R14).
    """
    ...
    for i, translations in enumerate(per_blade_translations):
        ...
        flat_blocks.append(translations.reshape(node_map.nodes_per_blade, 3))
...
def unflatten_translations(node_map: NodeOrderingMap, flat: np.ndarray) -> list[np.ndarray]:
    ...
    list of numpy.ndarray
        One ``(n_stations, 3, 3)`` array per blade, import order.
    """
    ...
    return [
        block.reshape(node_map.station_count, len(node_map.roles), 3)
