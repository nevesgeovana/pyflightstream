...
CONVENTIONS: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "Of the recorded rotor block the package reads position (the hub frame "
        "each rotor of the reference carries, <ALIAS>_SMRP) and the diameter behind the "
    ...
    (
        ...
        "down, q nose up, r nose right. The solver turns the free stream as a "
        "right-hand rotation about the frame axis it is given, so the emitted "
        "rotation takes the sign of its body axis in the geometry's frame: "
        "p = -omega_x, q = +omega_y, r = -omega_z, the same half turn about y "
        "(since 0.27.0, G13). Measured on 26.124: pitch by RPT-052, roll and "
        "yaw by the licensed probe T11 (build 8172026), RPT-060, which found "
        "the one sign of +1 used for all three axes reversed for roll and "
        "yaw, so a row of 0.21.0 to 0.26.0 stating roll_rate or yaw_rate was "
        "solved at the opposite rate. docs/post-processing-definitions.md is "
        "the definition "
        "of record. SCORED against the solver's own recorded output (RPT-063): "
        "the body-axis forces, the wind-axis drag and the stability and wind "
        "lift of the emitted polar row "
        "(tests/tier1_offline/test_goal028_axes_recorded_exports.py), and "
        "the body rate sense of all three axes "
        "(tests/tier1_offline/test_ops2011_rate_sense_against_recorded_probes.py). NOT "
