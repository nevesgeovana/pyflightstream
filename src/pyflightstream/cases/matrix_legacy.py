"""Legacy pipe-delimited run matrix: reader, converter, and run entry.
...
axis, also ``/``-separated; the legacy workflow varies one axis while
...
explicit override path is the only way to run the legacy MANUAL
...
def _resolve_build(
    ...
    legacy MANUAL mode); registry mode requires the active rows to
    ...
    if build.upper() == "MANUAL":
        raise LegacyMatrixError(
            "FS_BUILD is MANUAL, the legacy explicit-path mode: pass fs_exe=... "
...
def resolve_matrix(
    ...
    *,
    ...
    """Bind a legacy matrix to the workspace input library.
    ...
    ----------
    path : str or Path
        Legacy matrix location; only RUN = 1 rows resolve.
    workspace : CampaignWorkspace
    ...
    fs_exe : str or Path, optional
        ...
        registry and is the only way to run the legacy MANUAL mode.
    ...
    if not rows:
        raise LegacyMatrixError(f"{path} has no active rows (RUN = 1); nothing to resolve or run")
...
def plan_matrix(
    ...
    *,
    ...
    """Pre-flight a legacy matrix without executing anything.
...
def run_matrix(
    ...
    *,
    ...
    """Read a legacy matrix and run it: the one-call first-class entry.
