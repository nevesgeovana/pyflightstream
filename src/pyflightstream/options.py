...
``register_option`` model (D1 adoption of the 2026-07-23 library
review): every option is declared once with a default, a docstring,
and a validator, and every access goes through :func:`get_option` /
:func:`set_option`.

Two deliberate differences from pandas:
...
def register_option(
    ...
    if key in _REGISTRY:
        raise OptionError(
            f"option {key!r} is already registered; options are declared "
            "exactly once, at import of the module that owns them."
        )
...
def non_empty_path_text(value: Any) -> None:
    """Accept a non-empty string usable as a filesystem path."""
...
# CLI reads its scratch and timeout defaults here, so a licensed
# machine can retune them once per process instead of per command.

register_option(
    "qa.scratch_root",
    "runs",
    "Root directory of the gitignored local run scratch; the QA tiers "
...
register_option(
    "qa.probe_timeout_s",
    120.0,
    "Per-probe wall-clock limit in seconds of the Tier 2 validity "
...
register_option(
    "qa.case_timeout_s",
    900.0,
    "Per-point wall-clock limit in seconds of the Tier 3 physics and "
