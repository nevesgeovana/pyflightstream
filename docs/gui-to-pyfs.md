...
| Start a new simulation and import a mesh file in its length unit | row key `GEOMETRY` naming an `.obj` or `.stl`; sidecar table `[import]` with its `units`, and sidecar key `boundaries`, the file's surface names in its order, which the plan writes from an OBJ's groups ([starting from an OBJ or STL](mesh-inputs.md#starting-from-an-obj-or-stl)) | `NEW_SIMULATION`, `IMPORT`, `SET_SIMULATION_LENGTH_UNITS` | 26.101 to 26.124 (except `IMPORT`) |
