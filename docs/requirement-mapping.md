...
| C9 Optional-dependency error shape | NFR-25 | The reference choice was to elevate AD-05 to a tested requirement, which is what NFR-25 is |
...
| M4 Provenance and the PROV model | NFR-07 (part a) | The owning seat accepted the split (a) and DEFERRED the standardized export (b), so only (a) landed |
...
| SCOPE-fsi-classification | With the owning seat | Product-owner seat, needs no run |
| SCOPE-audience-tiebreak | With the owning seat | Product-owner seat, needs no run |
...
**It was resolved in FR-46's favour on 2026-08-03, and FR-37 was
