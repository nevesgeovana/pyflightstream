...
def test_the_reader_holds_no_import_inside_a_function_body():
    ...
    ``tests/tier1_offline/test_conventions.py`` owns the rule for the whole package;
...
def test_the_whole_chain_the_row_the_staged_copy_and_the_opened_path(tmp_path):
    ...
    # A .fsm point imported nothing, so its record writes NO mesh_import key:
    # a reader older than 0.27.0 refuses a key it does not know (G01).
    assert "mesh_import" not in records[0].model_dump(mode="json")
...
def test_an_obj_row_imports_the_staged_copy_in_the_unit_its_sidecar_declares(
    ...
    assert records[0].model_dump(mode="json")["mesh_import"] == {"units": units, "operations": []}
...
def _flat_rotor_matrix(tmp_path, origin):
    ...
    path = tmp_path / "flat_origin.fs"
    path.write_text(_FLAT_ORIGIN_HEADER + "\n" + "-" * 20 + "\n" + row + "\n", encoding="utf-8")
    return path


def test_a_flat_rotor_row_may_name_its_origin_by_a_reference_point(tmp_path):
    """THE DEFECT: a MOTIONS record may say ``ROTOR_ORIGIN: ERP1`` and have the
    point's coordinates bound from ``inputs/reference_points.toml``
    (PFS-2029.11.02), while the flat rotor row, the one-rotor form every
    campaign before 0.11.0 wrote, still demanded three numbers and refused the
    name. One rotor is not less entitled to a named hub than two; measured on
    the tier-3 workspace's installed row on 2026-09-08. The resolved case
    carries the coordinates under ``ROTOR_ORIGIN`` and the name beside them
    under ``ROTOR_ORIGIN_POINT``, exactly as a bound motion record does.
    """
    workspace = make_library(tmp_path, register_build=("26.120", "C:/fs/FS.exe"))
    (workspace.inputs_dir / "reference_points.toml").write_text(
        "[ARP]\nx_m = 2.0\n\n[ERP1]\nx_m = 4.5\ny_m = 0.0\nz_m = 0.0\n", encoding="utf-8"
    )
    stage_geometry(workspace, "rotor.fsm")
    resolved = resolve_matrix(
        _flat_rotor_matrix(tmp_path, "ERP1"),
        workspace,
        name="flat",
        fs_version="26.120",
        recipes={},
    ...
    assert case.variables["ROTOR_ORIGIN"] == "4.5,0.0,0.0"
    assert case.variables["ROTOR_ORIGIN_POINT"] == "ERP1"


def test_a_flat_rotor_row_naming_an_airframe_point_is_refused_naming_the_kind(tmp_path):
    workspace = make_library(tmp_path, register_build=("26.120", "C:/fs/FS.exe"))
    (workspace.inputs_dir / "reference_points.toml").write_text(
        "[ARP]\nx_m = 2.0\n\n[ERP1]\nx_m = 4.5\n", encoding="utf-8"
    )
    stage_geometry(workspace, "rotor.fsm")
    with pytest.raises(InputArtifactError, match="ARP") as caught:
        resolve_matrix(
            _flat_rotor_matrix(tmp_path, "ARP"),
            workspace,
            name="flat",
            fs_version="26.120",
            recipes={},
...
def test_a_rotor_row_run_through_the_workflow_leaves_its_reductions_beside_the_plots(
    ...
    assert sorted(manifest["skipped"]) == [
        ...
        # nothing to split, and the skip that stood here advised a new run that
        # would have recorded nothing more.
    ]
