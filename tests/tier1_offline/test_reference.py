...
def test_the_srs_data_model_table_agrees_with_the_status_legend():
    ...
    page = (Path(__file__).resolve().parents[2] / "docs" / "srs" / "data-model.md").read_text(
        encoding="utf-8"
    )
...
def test_the_build_page_counts_the_shared_release_names_it_actually_shares():
    ...
    # agreeing on a wrong answer: the seven 26.1x builds all print 26.1.
    ...
    # six until 26.124's identity report of 2026-09-14 did the same.
    assert _sharing_a_printed_name(versions) == 7
