...
def test_a_workspace_that_still_carries_an_empty_parsed_folder_is_untouched(tmp_path):
    ...
    assert workspace.collect_outputs("9001", [produced], datapoint=PointName("AL+000")) == [
...
def test_collect_outputs_moves_declared_files_into_outputs(tmp_path):
    ...
    workspace = CampaignWorkspace(tmp_path / "camp")
    collected = workspace.collect_outputs("9001", [produced], datapoint=PointName("AL+000"))
    assert collected == ["datapoints/DP-AL+000/loads.txt"]
    assert not produced.exists()
    assert (
        tmp_path / "camp" / "sims" / "sim_9001" / "datapoints" / "DP-AL+000" / "loads.txt"
    ).is_file()
...
def test_collect_refuses_missing_declared_outputs(tmp_path):
    ...
    with pytest.raises(WorkspaceError, match="FAILED_INCOMPLETE_OUTPUT"):
        workspace.collect_outputs(
            "9001", [tmp_path / "never_written.txt"], datapoint=PointName("AL+000")
        )
...
#: none of its content.
#:
#: EVERY VALUE HERE IS CHOSEN FOR THE EXAMPLE, and the earlier wording of
#: this note is why it says so twice. That wording split the block into a
#: transcribed vocabulary and invented magnitudes, which reads as a
#: promise that the sense of rotation and the two signs came off a real
#: artifact: four values, one bit each, and exactly the kind of thing
#: invariant 5 keeps out whether or not any one of them identifies
#: anything. What is transcribed is the SHAPE, that a campaign records
#: its sense in the published inboard vocabulary and records two measured
#: signs at all.
#:
#: The values are the ones docs/workspace-and-workflows.md publishes, put
#: there by the same change rather than looked up in it, and the two are
#: kept together on purpose: the page's block is validated against this
#: model by tests/tier1_offline/test_docs_example_currency.py, so a reader who finds
#: these numbers in both places is looking at one artifact and not at a
#: coincidence.
PROPELLER_REFERENCE_TOML = """
area_m2 = 10.0
chord_m = 1.2
span_m = 8.0
...
def _artifact_id(field: str, parameter: object) -> str:
    """An id that names its case, and that two cases cannot share.

    The first version built ids from `abs(magnitude)` and `len(written)`,
    which discards the sign and collides "1.0" with a quoted "1". Nothing
    tested the wrong thing, because `tmp_path` is per-invocation, and that
    is the trap: hoisting `library(tmp_path)` to a module-scoped fixture,
    which is the natural cost optimisation here, would have two cases
    overwrite each other's artifact and the survivor test the other's
    content while passing.
    """
    return "r" + re.sub(r"\W", "", f"{field}{parameter}").lower()
...
def _reference_with(tmp_path, artifact_id: str, line: str, replacing: str) -> str:
    ...
    """
    workspace = library(tmp_path)
    assert PROPELLER_REFERENCE_TOML.count(replacing) == 1, replacing
    body = PROPELLER_REFERENCE_TOML.replace(replacing, line)
...
def test_no_module_outside_the_model_reads_the_recorded_rotor_block():
    ...
    package does. Documentation is not a guard, which is this
    repository's own rule.

    The shape is the one used by
    `test_no_parse_back_api_exists_anywhere_in_the_workspace_package`
    above: an absence asserted across the package, failing with the
    ...
    # the source and immediately reported `script/helpers.py`, which names
    # `blade_travel` inside a refusal MESSAGE so a caller holding that
    # vocabulary is routed to the field that records it. A mention in a
    ...
    for module in package.rglob("*.py"):
        ...
        where_module = module.relative_to(package).as_posix()
        for node in ast.walk(tree):
            # `lineno` is computed per BRANCH rather than up here, because
            ...
            if isinstance(node, ast.Attribute) and node.attr in watched:
                ...
                offenders.append(
                    f"{where_module}:{node.lineno} {node.func.id}s {node.args[1].value!r}"
                )
    assert scanned > 50, (
        f"the scan reached only {scanned} modules, and an empty scan passes this "
...
def test_default_naming_renders_the_point_name():
    """0.21.0: `{point}` is the point name the run hands over, and nothing else."""
    naming = NamingTemplate()
    stem = naming.render_point(
        campaign="camp", sim="9001", point={"alpha": 2.0, "beta": 0.0}, name="AL+020BE+000"
    )
    assert stem == "AL+020BE+000"
...
def test_output_names_can_carry_the_point_name():
    naming = NamingTemplate()
    name = naming.render_output(
        "loads_{point}.txt", campaign="camp", sim="9001", point={"alpha": 2.0}, point_name="AL+020"
    )
    assert name == "loads_AL+020.txt"
...
def test_unportable_rendered_name_is_refused():
    ...
    with pytest.raises(NamingTemplateError, match="portable"):
        naming.render_point(campaign="my run", sim="9001", point={"alpha": 2.0}, name="AL+020")
...
def test_an_ordinary_output_name_still_passes_through(tmp_path):
    ...
    assert (
        template.render_output(
            "loads_{point}.txt", campaign="c", sim="1", point={"alpha": 2.0}, point_name="AL+020"
        )
        == "loads_AL+020.txt"
    )
...
def test_collecting_onto_an_existing_collected_name_is_refused(tmp_path):
    ...
    with pytest.raises(WorkspaceError, match=re.escape("already in datapoints/DP-AL+000/")):
        workspace.collect_outputs("1", [tmp_path / "loads.txt"], datapoint=PointName("AL+000"))
    held = workspace.sim_dir("1") / "datapoints" / "DP-AL+000" / "loads.txt"
...
def test_output_digests_hash_what_collection_produced(tmp_path):
    ...
    assert digests["datapoints/DP-AL+000/loads.txt"] == hashlib.sha256(b"LOADS").hexdigest()
...
def test_collect_refuses_a_source_inside_a_managed_subdirectory(tmp_path):
    ...
    source = sim / "datapoints" / "DP-AL+000" / "already_collected.txt"
    source.parent.mkdir(parents=True)
    source.write_text("evidence", encoding="utf-8")

    with pytest.raises(WorkspaceError, match="datapoints"):
        workspace.collect_outputs("A", [source], datapoint=PointName("AL+000"))
...
def test_collect_refuses_another_simulations_collected_evidence(tmp_path):
    ...
    source = other / "datapoints" / "DP-AL+000" / "loads.txt"
    source.parent.mkdir(parents=True)
...
def test_collect_refuses_a_path_that_climbs_back_in(tmp_path):
    ...
    workspace, sim = _prepared(tmp_path)
    other = workspace.create_sim("OTHER")
    held = other / "datapoints" / "DP-AL+000" / "loads.txt"
    held.parent.mkdir(parents=True)
    held.write_text("another run's numbers", encoding="utf-8")
    climbing = sim / ".." / "sim_OTHER" / "datapoints" / "DP-AL+000" / "loads.txt"

    with pytest.raises(WorkspaceError, match="belongs to sim_OTHER"):
        workspace.collect_outputs("A", [climbing], datapoint=PointName("AL+000"))
    assert held.is_file()
...
def test_collect_still_takes_an_unmanaged_subfolder_of_the_simulation(tmp_path):
    ...
    assert workspace.collect_outputs("A", [source], datapoint=PointName("AL+000")) == [
        "datapoints/DP-AL+000/loads.txt"
    ]
    assert (sim / "datapoints" / "DP-AL+000" / "loads.txt").is_file()
...
def test_archive_does_not_cross_a_staged_link(tmp_path):
    ...
    for sim_id in ("9001", "9002"):
        ...
        collected = workspace.sim_dir(sim_id) / "datapoints" / "DP-AL+000"
    ...
    with zipfile.ZipFile(bundle) as archive:
        ...
        assert "datapoints/DP-AL+000/loads.txt" in names
...
def test_goal019_bandd_a_collision_on_the_second_file_moves_neither(tmp_path):
    ...
    point = PointName("AL+000")
    # An earlier run of this point already left a second.txt behind.
    held = workspace.sim_dir("9001") / "datapoints" / "DP-AL+000"
...
def test_goal019_bandd_a_collection_that_would_have_worked_still_does(tmp_path):
    ...
    assert workspace.collect_outputs("9001", produced, datapoint=PointName("AL+000")) == [
        "datapoints/DP-AL+000/first.txt",
        "datapoints/DP-AL+000/second.txt",
    ]
