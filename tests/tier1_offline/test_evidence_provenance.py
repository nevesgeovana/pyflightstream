...
GOLDENS = REPO / "tests" / "tier1_offline" / "goldens"
...
def _load_generator():
    ...
    ``tests/tier1_offline/test_extras.py`` loads ``scripts/chm_to_pdf.py`` the same way
...
def test_no_surface_that_reaches_a_person_asks_the_weaker_question():
    ...
    root = Path(__file__).parents[2] / "src" / "pyflightstream"
    person_facing = {
...
def test_the_committed_enumeration_matches_what_the_emitter_refuses():
    ...
    assert committed_files, (
        "no absent-command enumeration is committed under tests/tier1_offline/goldens/. One is "
