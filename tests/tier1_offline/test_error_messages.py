...
# `tests/tier1_offline/test_workspace.py` because the two modules pin different things: that
...
def test_foreign_header_names_the_verified_layout(tmp_path):
    ...
    with pytest.raises(
        MatrixError,
        match=r"header does not match the verified 19-column layout; expected ",
    ):
        read_matrix(bad)
...
def _rotor_row_against(geometry, cell):
    from tests.tier1_offline.test_workflows import _rotor_row
