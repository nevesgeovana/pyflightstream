...
def test_the_page_cites_the_executed_test_its_example_comes_from():
    ...
    assert "tests/tier1_offline/test_matrix_run.py" in text
    source = SOURCE_TEST.read_text(encoding="utf-8")
    assert f"def {LIFTED_FROM}(" in source, (
        f"the page cites {LIFTED_FROM}, which no longer exists in "
...
def test_the_recorded_rotor_block_on_the_page_is_the_one_the_suite_validates():
    ...
    from tests.tier1_offline import test_workspace
...
def test_the_matrix_example_does_not_deny_the_capability_it_now_has():
    ...
    it: the example is registered in ``tests/tier1_offline/test_examples.py`` with an
