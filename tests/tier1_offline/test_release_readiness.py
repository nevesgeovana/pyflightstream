...
Usage, and it is the whole interface::
    ...
    from tests.tier1_offline.test_release_readiness import missing_last_step_evidence
...
``tests/tier1_offline/goldens/absent_on_26123.txt`` and not repeated here,
...
def absent_command_count(root: Path) -> int:
    ...
    -------
    int
        Number of command names in ``tests/tier1_offline/goldens/absent_on_26123.txt``.
    """
    path = root / "tests" / "tier1_offline" / "goldens" / f"absent_on_{LAST_STEP_STEM}.txt"
...
def test_the_absent_count_is_reported_and_gates_nothing():
    ...
    path = REPO / "tests" / "tier1_offline" / "goldens" / f"absent_on_{LAST_STEP_STEM}.txt"
    stamped = [
