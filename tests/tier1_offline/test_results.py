...
from tests.tier1_offline.conftest import REPO
...
def test_a_probe_export_declaring_no_point_reads_as_an_empty_table():
    ...
    assert report.current_iteration == 89
    assert report.reported_build == "8112026"
...
#
# THE GUARD LIVES HERE rather than in tests/tier1_offline/test_conventions.py, whose
...
#: ratchet in the shape `MYPY_EXEMPTIONS` uses (tests/tier1_offline/test_traceability.py):
...
def _internal_import_closure(roots) -> set[str]:
    ...
    """
    from tests.tier1_offline.test_conventions import _imported_module_names
...
def test_every_new_parser_reads_the_line_ending_the_solver_actually_writes():
    ...
    `tests/tier1_offline/test_matrix.py` pins `tests/tier1_offline/fixtures/** text eol=lf`,
    so every
    ...
    for name, parser in cases:
        ...
        # tests/tier1_offline/test_matrix.py, which reads the git attributes.
        crlf = text.replace("\n", "\r\n")
