...
def test_resolve_matrix_applies_reference_and_setup_to_the_cases(tmp_path):
    ...
    assert by_sim["9001"].variables["legacy_ref"] == "003"
    assert by_sim["9001"].variables["legacy_set"] == "003"
...
def test_registry_build_resolves_the_executable(tmp_path):
    ...
    resolved = resolve_matrix(
        REGISTRY_FIXTURE, workspace, name="legacy", fs_version="26.120", recipes=RECIPES
    )
...
def test_manual_build_requires_the_explicit_override(tmp_path):
    workspace = make_library(tmp_path)
    with pytest.raises(LegacyMatrixError, match="MANUAL.*fs_exe"):
        resolve_matrix(FIXTURE, workspace, name="legacy", fs_version="26.12", recipes=RECIPES)
...
def test_mixed_builds_are_refused(tmp_path):
    ...
    with pytest.raises(LegacyMatrixError, match="2 FS_BUILD values"):
        resolve_matrix(matrix, workspace, name="legacy", fs_version="26.12", recipes=RECIPES)
...
def test_run_matrix_executes_and_records_every_point(tmp_path):
    ...
    assert [record.run_id for record in records] == [
        "legacy/sim_8001/a+00.0",
        "legacy/sim_8001/a+02.0",
        "legacy/sim_8002/b-03.0",
        "legacy/sim_8002/b+03.0",
    ]
...
def test_run_matrix_refuses_a_blocked_preflight_before_any_execution(tmp_path):
    workspace = make_library(tmp_path, register_build=("26.120", "C:/fs/FS.exe"))
    with pytest.raises(LegacyMatrixError, match="nothing was\\s+executed"):
        run_matrix(
            REGISTRY_FIXTURE,
            workspace,
            name="legacy",
...
def test_matrix_without_active_rows_is_refused(tmp_path):
    ...
    with pytest.raises(LegacyMatrixError, match="no active rows"):
        resolve_matrix(matrix, workspace, name="legacy", fs_version="26.120", recipes=RECIPES)
