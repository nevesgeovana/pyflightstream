...
def test_a_matching_point_binds_with_no_mismatch():
    binding = bind_conditions({"alpha": 2.0, "beta": 0.0, "velocity": 30.0}, FakeReport())
...
def test_print_rounding_is_inside_the_tolerance_and_a_real_offset_is_not():
    ...
    """
    rounding = bind_conditions({"alpha": 2.0004}, FakeReport(angle_of_attack_deg=2.0))
    assert rounding.matched, "half a printed count must not be called a mismatch"

    real = bind_conditions({"alpha": 2.001}, FakeReport(angle_of_attack_deg=2.0))
    assert not real.matched
...
def test_the_reviews_own_reproduction_is_refused():
    ...
    binding = bind_conditions({"alpha": 0.0}, FakeReport(angle_of_attack_deg=2.0))
...
def test_an_axis_the_export_does_not_print_is_recorded_as_unchecked():
    ...
    binding = bind_conditions({"alpha": 2.0, "beta": 1.0}, report)
    assert binding.unprinted == ("beta",)
    assert {check.axis for check in binding.checks} == {"alpha"}
    assert binding.matched, "an unprintable axis is unchecked, not failed"


def test_an_axis_the_campaign_does_not_sweep_is_simply_absent():
    binding = bind_conditions({"alpha": 2.0}, FakeReport())
    assert {check.axis for check in binding.checks} == {"alpha"}
...
def test_a_unit_confusion_is_a_mismatch_rather_than_a_match():
    ...
    binding = bind_conditions({"velocity": 58.3196}, FakeReport(freestream_velocity_m_s=30.0))
    assert not binding.matched
    assert binding.mismatches[0].unit == "m/s"


def test_a_requested_value_of_none_is_not_requested():
    binding = bind_conditions({"alpha": None, "velocity": 30.0}, FakeReport())
    assert {check.axis for check in binding.checks} == {"velocity"}


def test_an_empty_binding_is_not_evidence_of_agreement():
    """`matched` is vacuously true with nothing to compare, which is why
    the assessor records the checks rather than the boolean."""
...
def test_every_declared_binding_is_driven_by_at_least_one_case(axis, attribute, tolerance, unit):
    ...
    binding = bind_conditions({axis: baseline + 10 * tolerance}, report)
...
def test_the_records_carry_the_whole_decision():
    ...
    (record,) = bind_conditions({"alpha": 0.0}, FakeReport(angle_of_attack_deg=2.0)).as_records()
    assert record == {
        "axis": "alpha",
        "requested": 0.0,
        "reported": 2.0,
