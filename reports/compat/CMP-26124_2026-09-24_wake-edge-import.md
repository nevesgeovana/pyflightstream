...
| Package | pyflightstream 0.27.0.dev2 |
| Solver identity lines | FlightStream version 26.1, build #8172026 |

## Summary

1 verified, 0 broken, 0 removed, 370 unprobed.
...
| IMPORT_WAKE_EDGES_FROM_FILE | verified | script processing continued past the command, no error between the sentinels, and the effect was observed: the solver logs 16 trailing edges imported for boundary Wing, one per trailing-edge mesh edge of the wing, a line it prints only when the import marks something (RPT-061) |
...
| ROTATE_SURFACE | unprobed | not probed in this run |
...
| SOLVER_TIME_AVERAGING | unprobed | not probed in this run |
| SET_NEW_UNSTEADY_SOLVER_ACTION | unprobed | not probed in this run |
