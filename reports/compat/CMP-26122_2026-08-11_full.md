# Compat report: FlightStream 26.122 (2026-08-11)
...
| Solver identity lines | FlightStream version 26.1, build #8092026 |

## Summary

83 verified, 1 broken, 0 removed, 291 unprobed.
...
| SET_OUTFLOW_TRAILING_EDGES | unprobed | no probe specification for this command yet |
...
| AUTO_DETECT_WAKE_TERMINATION_NODES | unprobed | the command ran without a script abort or logged error, but its effect is not observable with the current instruments; asserted effect: the saved simulation does not move on the probe geometry (RPT-020); a clean synthetic wing may simply have no wake termination nodes to find, so this needs a geometry that does |
...
| AIR_ALTITUDE | verified | script processing continued past the command, no error between the sentinels, and the effect was observed: the settings dump reports the 5000 m standard-atmosphere density (0.736 kg/m^3); the first full sweep observed 1.056 kg/m^3 instead, which is the 5000 ft standard state, so the METERS units argument reads ignored |
| DETECT_TRAILING_EDGES_BY_SURFACE | unprobed | no probe specification for this command yet |
| TRAILING_EDGES_IMPORT | unprobed | no probe specification for this command yet |
...
| SET_SCENE_CONTOUR | unprobed | no probe specification for this command yet |
