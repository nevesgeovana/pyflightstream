...
- **The Zenodo archive row of v0.22.0 is OWED and does not exist yet.** The
  version DOI is minted from the GitHub RELEASE object and not from the tag, and
  creating that release is a manual step, so the identifier this tag will carry
  is missing from CITATION.cff at the moment the tag is cut and lands one commit
  after it.
