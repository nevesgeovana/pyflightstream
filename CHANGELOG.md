...
- **The v0.17.0 archive row is OWED in CITATION.cff.** A version DOI is minted
  from the GitHub release, so the identifier is recorded ONE COMMIT AFTER the
  tag that names it; this is that window, and the section says the row is owed
  rather than leaving a shipped release quietly uncitable. Cite v0.17.0 by the
  concept DOI until it lands: it resolves to the newest archived version.
...
- **One type-checker exemption is REMOVED, the first this project has
  ...
  it: mypy recount 2026-09-11: 370 errors in 18 of 83 modules, where the
