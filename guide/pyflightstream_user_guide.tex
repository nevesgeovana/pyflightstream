...
for FlightStream 26.000. Recorded evidence: 26.101 (documented),
26.120 (verified), 26.121 (verified). Earlier versions await
release-notes review or backfill probing.
\end{lstlisting}
  \begin{itemize}
...
\begin{frame}{Hotfix inheritance}
  \begin{itemize}
    ...
    \item \code{26.121} is no longer the hypothetical example: it started from every \code{26.120} record, and the licensed probe on that build then contradicted one of them. AIR\_ALTITUDE is {\color{statusred}broken} on 26.120 and {\color{statusblue}verified} on 26.121.
...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \begin{tabular}{@{}p{5.4cm}p{1.2cm}p{6.0cm}@{}}
    ...
    \midrule
    AIR\_ALTITUDE ignores its units argument and applies feet & 26.120 & set the fluid state through FLUID\_PROPERTIES, or move to 26.121, where the same probe passes \\
    SET\_MOTION\_START\_TIME aborts the script & both & converge the base flow another way; the 26.121 re-probe found the same abort \\
