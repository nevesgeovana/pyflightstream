...
# Every command any registered manual edition documents is
# recorded, so this is usually a spelling error or a command from a
# build this install does not register.
...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \begin{tabular}{@{}p{5.4cm}p{1.2cm}p{6.0cm}@{}}
    ...
    \midrule
    AIR\_ALTITUDE does not produce the altitude asked for & 26.100, 26.101, 26.120 & set the fluid state through FLUID\_PROPERTIES, or move to 26.121 or 26.122, where the same probe passes \\
...
pyfs-manual register     --editions MANIFEST --build 26.123   # carries a build forward
\end{lstlisting}
  \begin{itemize}
