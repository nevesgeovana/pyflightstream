...
\institute[]{pyflightstream 0.20.0.dev0, MIT licensed}
\date{September 2026}
...
\begin{frame}[fragile]{Versions: the YY.XXX canonical scheme}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item A vendor name can also name more than one build: 26.120, 26.121, 26.122 and 26.123 all ship as "26.12", and since 2026-08-04 so do 26.100 and 26.101 as "26.1". Resolution refuses such a name and lists the candidates, rather than returning one of them. Note what that means for a script you wrote earlier: a vendor name is unique only until the vendor ships the next build under it, so pass the canonical identifier.
...
resolve("26.12")           # Ambiguous: 26.120, 26.121, 26.122 or 26.123?
...
\end{lstlisting}
    ...
    \begin{tikzpicture}[node distance=6mm]
      ...
      \node[below=3mm of f,text width=36mm,font=\tiny,align=center]
...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \begin{tabular}{@{}p{5.4cm}p{1.2cm}p{6.0cm}@{}}
    ...
    NEW\_OFF\_BODY\_STREAMLINE aborts the script & 26.120, 26.121, 26.122, 26.123 & use probe lattices for off-body data \\
