...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \vspace{2mm}
  \begin{itemize}
    \item Each carries {\color{statusred}\textbf{broken}} status for the builds named, with the committed report cited, and the builder REFUSES to emit them for those builds (\code{BrokenCommandError}, FR-48): a broken command is one a probe watched fail, and unlike an absent one it produces a complete run with wrong numbers in it. If a run needs one anyway, and re-probing it is the run that does, \code{script.allow\_broken(NAME, reason=...)} emits it and records the command, the version, the report and your justification in the script and in the run manifest.
