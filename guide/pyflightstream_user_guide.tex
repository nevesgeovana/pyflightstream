...
\institute[]{pyflightstream 0.17.0, MIT licensed}
\date{September 2026}
...
\begin{frame}{Three tiers of quality assurance behind the statuses}
  ...
  \begin{itemize}
    ...
    \item Tier 3 asks: does the whole workspace run, and does the physics still hold? \code{tests/tier3\_licensed} is a campaign workspace of seven run matrices over a synthetic library, one test per row; the physics cases are rows of it, judged against their committed references, and a cross-version drift suite compares solver builds on identical inputs.
  \end{itemize}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}
    ...
    \item \code{pyfs-workspace}, and \code{pyfs-matrix convert} and \code{plan}, run anywhere: they scaffold the workspace tree, turn a run matrix into a campaign, and pre-flight one without a solver. \code{pyfs-matrix run} STARTS THE SOLVER and needs a licensed machine like the \code{pyfs-qa} tools.
