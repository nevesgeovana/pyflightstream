...
\end{lstlisting}
  \begin{alertblock}{Why explicit properties}
    The altitude form (AIR\_ALTITUDE, SRC-003 p.328) was probed broken on 26.120 and applies feet regardless of the units argument: 5000 METERS produced 1.056 kg/m$^3$, the 5000 ft standard state, where 0.736 is correct. A density 43\% too high, with no error (committed compat report). The 26.121 hotfix fixes it and the probe on that build promoted it to {\color{statusblue}\textbf{verified}}. On 26.120, use FLUID\_PROPERTIES; the matrix carries the status per build, which is the point of recording the hotfix digit.
  \end{alertblock}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}
    ...
    \item Conversion carries the matrix codes, not output names: the matrix has no such column. Add \code{outputs = ["loads\_\{point\}.txt"]} to each converted \code{[[sim]]} before running, because the loop collects only declared outputs and a swept case needs one name per point.
  \end{itemize}
\end{frame}
...
\begin{frame}{Hotfix inheritance}
  \begin{itemize}
    ...
    \item Read an inherited cell for what it is. The matrix does not yet mark which records were probed on that build and which were carried over, so a hotfix cell means "the base release's evidence, unless a probe on this build says otherwise".
  \end{itemize}
  \begin{block}{Why the identifiers carry the hotfix digit}
    Vendor hotfix builds change solver behavior, and here the change was a silent numerical one: the same command, the same script, a density 43\% apart. Without the hotfix digit the two builds would share one record and one of the two answers would be wrong with nothing to say so.
  \end{block}
\end{frame}
...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \begin{itemize}
    ...
    \item Read the last row precisely: SWEEPER\_REF\_VELOCITY\_SAME is {\color{statusgreen}documented} on 26.120, which records the manual's word and no probe. It was first probed on 26.121, where it aborts. That is not evidence the hotfix broke it, and the guide does not claim so.
