...
\institute[]{pyflightstream 0.7.0, MIT licensed}
\date{August 2026}
...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \begin{tabular}{@{}p{5.4cm}p{1.2cm}p{6.0cm}@{}}
    ...
    \midrule
    AIR\_ALTITUDE does not produce the altitude asked for & 26.100, 26.101, 26.120, 26.122* & set the fluid state through FLUID\_PROPERTIES, or move to 26.121, the one registered build where the same probe passes \\
    SET\_MOTION\_START\_TIME aborts the script & 26.101, 26.120, 26.121, 26.122* & converge the base flow another way; every build probed since has found the same abort, the May one for the first time on 2026-08-08 \\
  ...
  \begin{itemize}
    ...
    \item An asterisk marks 26.122, where the status is INHERITED from 26.120 rather than probed: that build was registered on 2026-08-10 and no command has been measured on it, so the builder refuses these there on the strength of its base release's runs. Read the Builds column as what the emitter will refuse, which is what you need to know, and not as a list of builds somebody watched fail.
