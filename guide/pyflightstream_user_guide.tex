...
\institute[]{pyflightstream 0.11.1, MIT licensed}
\date{August 2026}
...
\begin{frame}{Three tiers of quality assurance behind the statuses}
  ...
  \begin{tabular}{@{}lllp{4.2cm}@{}}
    ...
    \midrule
    1 & schema, builder goldens, parsers & CI, no solver & green test suite in continuous integration, no solver needed \\
...
\end{lstlisting}
  ...
  \begin{itemize}
    ...
    \item \textbf{Five further keys PIN the fluid} (v0.11.0): \code{RHOkgm3}, \code{MUPas}, \code{ASMPS} (speed of sound), \code{TK} and \code{PPA}, each replacing what the standard atmosphere would supply and each recorded as pinned. From v0.11.1 they may instead live in the setup artifact the row's \code{SET} names, under a \code{[flight\_condition]} table, so a thirteen-point polar states them once rather than thirteen times; a row that states one wins over the setup's. The velocity keys and \code{REmi} may NOT go there, because they are what makes a point that point.
