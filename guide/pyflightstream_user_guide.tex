...
\end{lstlisting}
  ...
  \end{enumerate}
  \begin{alertblock}{Pitfalls}
    SET\_MOTION\_START\_TIME was probed on 26.120 and found to abort scripts (committed compat report, broken status): the log exported before it exists and the one after it never appears. The re-probe ran on 26.121 and it aborts there too, so this is not something the hotfix repaired: converge the steady base flow by other means. A positive start time is exactly the case both probes covered.
  \end{alertblock}
\end{frame}
