...
\institute[]{pyflightstream 0.14.1, MIT licensed}
\date{September 2026}
...
\begin{frame}[fragile]{Recipe 7 addendum: migrating a run matrix}
  \begin{itemize}
    \item Coming from spreadsheet-driven run matrices (the pipe-delimited layout with AL and BE sweep codes)? \textbf{First upgrade the file.} v0.9.0 removed the \code{RE} and \code{MACH} columns and replaced them with one mandatory \code{FLIGHT\_CONDITION} cell, so a matrix written under either older layout is recognised and refused rather than misread:
  \end{itemize}
\begin{lstlisting}
