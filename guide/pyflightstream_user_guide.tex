...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \begin{tabular}{@{}p{5.4cm}p{1.2cm}p{6.0cm}@{}}
    ...
    \midrule
    AIR\_ALTITUDE ignores its units argument and applies feet & 26.100, 26.101, 26.120 & set the fluid state through FLUID\_PROPERTIES, or move to 26.121, the one registered build where the same probe passes \\
    SET\_MOTION\_START\_TIME aborts the script & 26.101, 26.120, 26.121 & converge the base flow another way; every build probed since has found the same abort, the May one for the first time on 2026-08-08 \\
    NEW\_OFF\_BODY\_STREAMLINE aborts the script & both & use probe lattices for off-body data \\
