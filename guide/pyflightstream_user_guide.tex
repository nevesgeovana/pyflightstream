...
\begin{frame}[fragile]{Versions: the YY.XXX canonical scheme}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item A vendor name can also name more than one build: 26.120, 26.121 and 26.122 all ship as "26.12", and since 2026-08-04 so do 26.100 and 26.101 as "26.1". Resolution refuses such a name and lists the candidates, rather than returning one of them. Note what that means for a script you wrote earlier: a vendor name is unique only until the vendor ships the next build under it, so pass the canonical identifier.
...
resolve("26.12")           # Ambiguous: 26.120, 26.121 or 26.122?
...
\end{lstlisting}
    ...
    \begin{tikzpicture}[node distance=6mm]
      ...
      \node[below=3mm of e,text width=36mm,font=\tiny,align=center]
        {one vendor name, "26.12", names both of the last two; resolution refuses it and lists them};
    \end{tikzpicture}
    \end{center}
  \end{columns}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}
    ...
    \item It lists builds by REACHABILITY rather than by row, which is why the newest is marked inherited: a hotfix carries its base release's records wherever it has none of its own. Built from the rows alone this list hid the newest build entirely, so a caller weighing a switch was never offered the one that would have served them.
