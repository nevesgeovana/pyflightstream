...
\begin{frame}[fragile]{Installation and requirements}
  \begin{itemize}
    \item Python 3.11 or newer. Runtime dependencies: numpy, pandas, pydantic, pyyaml, trimesh, xarray.
...
\begin{frame}{Three tiers of quality assurance behind the statuses}
  ...
  \begin{itemize}
    ...
    \item Tier 3 asks: does the whole workspace run, and does the physics still hold? \code{tests/tier3\_licensed} is a campaign workspace of six run matrices over a synthetic library, one test per row; the physics cases are rows of it, judged against their committed references, and a cross-version drift suite compares solver builds on identical inputs.
  \end{itemize}
\end{frame}
