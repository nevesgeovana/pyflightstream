...
\institute[]{pyflightstream 0.1.0, MIT licensed}
\date{July 2026}
...
\begin{frame}{Guide roadmap}
  ...
  \begin{description}
    ...
    \item[Part II] Hands on: your first validated script in ten minutes, and the tour of the fourteen curated helpers.
...
\begin{frame}{The problem this library solves}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item The command set changes between solver versions, and the changelog is incomplete.
      \item Failures are often silent: a missing script file exits with code 0 and produces nothing; a wrong argument may be ignored.
    ...
    \begin{alertblock}{The failure mode}
      You upgrade the solver, your old script still "runs", and three weeks later you discover half the outputs were built with a command the new version quietly ignores.
    \end{alertblock}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Installation and requirements}
  \begin{itemize}
    \item Python 3.11 or newer. Runtime dependencies: numpy, pandas, pydantic, pyyaml.
    \item Install from the repository (not yet on PyPI; planned for the public release):
  \end{itemize}
...
\begin{frame}{The command database and its four evidence statuses}
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item 116 commands, one YAML file per manual chapter, loaded and validated at import.
...
\begin{frame}{Three tiers of quality assurance behind the statuses}
  ...
  \vspace{3mm}
  \begin{itemize}
    \item Tier 2 asks: does this command do what the manual says, on this build? 64 commands verified and 4 found broken on 26.120.
...
\begin{frame}{The layered architecture}
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[node distance=3.2mm,
      ...
      \node[lay,below=of qa] (run) {\textbf{run / files}: executor, campaign loop, workspace};
      \node[lay,below=of run] (cases) {\textbf{cases}: SIM model, sweeps, legacy matrix};
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \item a whole campaign (\code{cases} + \code{files}).
      \end{itemize}
...
\begin{frame}{What each layer owns}
  ...
  \begin{tabular}{@{}lp{5.2cm}p{4.6cm}@{}}
    ...
    script & validating emission, phase order, layouts, helpers & \code{Script}, 14 curated helpers \\
    ...
    files & managed run workspace and manifest & \code{CampaignWorkspace}, \code{RunRecord} \\
  ...
  \vspace{2mm}
  \begin{itemize}
    \item \code{post} (labeled result arrays) and \code{fsi} (fluid structure seam) are reserved seams for the next milestones.
  \end{itemize}
\end{frame}
...
\begin{frame}{Why helpers, and why only fourteen}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{itemize}
    ...
    \column{0.43\textwidth}
    \small
    \begin{block}{The fourteen}
      free\_stream, atmosphere, solver\_settings, initialize\_solver, sweep, analysis\_setup, export\_results, probe\_points, probe\_line, probes\_from\_file, export\_probes, actuator\_disc, rotary\_motion, unsteady\_solver
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Recipe 7 addendum: migrating a legacy run matrix}
  \begin{itemize}
    \item Coming from spreadsheet-driven run matrices (the 15-column layout with AL and BE sweep codes)? The reader converts them losslessly:
  \end{itemize}
\begin{lstlisting}
from pyflightstream.cases.matrix_legacy import convert_matrix
convert_matrix("run_matrix.csv", "campaign.toml")
# unknown legacy codes are preserved as legacy_* variables,
...
\begin{frame}{Reading the compatibility matrix}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Current evidence (v0.1.0): 26.120 fully covered (64 {\color{statusblue}verified}, 47 {\color{statusgreen}documented}, 4 {\color{statusred}broken}, 1 removed); 26.100 partially backfilled (28 documented); 26.000 registered, honestly empty.
    \end{itemize}
    \column{0.43\textwidth}
...
\begin{frame}{Pitfalls found by the probe suite (Tier 2)}
  ...
  \begin{tabular}{@{}p{5.4cm}p{7.2cm}@{}}
    ...
    NEW\_SURFACE\_SECTION\_ DISTRIBUTION aborts the script & use the section export family instead \\
    \bottomrule
  \end{tabular}
  \vspace{2mm}
  \begin{itemize}
    \item All four carry {\color{statusred}\textbf{broken}} status in the database with the committed report cited; the builder still emits them (they are grammatically valid), but the matrix and the reference warn you.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Cheat sheet: the QA command line}
\begin{lstlisting}[style=txt]
...
\end{lstlisting}
  \begin{itemize}
    \item All of these require a licensed machine and write dated report pairs (markdown plus YAML) under \code{reports/}.
    \item You only need them when contributing evidence; as a user, you consume their committed results through the matrix.
  \end{itemize}
\end{frame}
