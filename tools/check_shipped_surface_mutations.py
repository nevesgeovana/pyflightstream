...
def make_repo(
    ...
    *,
    ...
    for args in (["init", "-q"], ["add", "-A"]):
        done = subprocess.run(["git", *args], cwd=str(root), capture_output=True, check=False)
...
def run(checker: Path, *args: str) -> subprocess.CompletedProcess[bytes]:
    return subprocess.run([sys.executable, str(checker), *args], capture_output=True, check=False)
...
CASES: list[tuple[str, str, int, str]] = [
    ...
    # The measured shape: a src-layout project's egg-info sits at
    # src/<name>.egg-info/, the root-anchored pattern could not span the
    # separator, and one real repository carried 7 permanent false findings
    # in a set that is deliberately not narrowable from a config.
    (
...
_CONFIG_VARIANTS = {
    ...
    "tree_only_no_floors": ("exempt-path: LICENSE\nexempt-path: README.md\nexempt-tree: docs/\n"),
    "hash_in_value": GOOD_CONFIG.replace("exempt-tree: docs/", "exempt-tree: docs/#draft/"),
}
...
def build_case(name: str, tmp: Path) -> list[str]:
    ...
    config.write_text(_CONFIG_VARIANTS.get(name, GOOD_CONFIG), encoding="utf-8", newline="\n")
    ...
    if name == "dirty_wheel":
        dist = make_dist(
            tmp,
            _replace(CLEAN_WHEEL, "pkg/core.py", DIRTY_DOCSTRING.encode()),
            CLEAN_SDIST,
        )
    ...
    if name == "dirty_stray_nul":
        ...
        dist = make_dist(tmp, _with(CLEAN_WHEEL, **{"pkg/notes.md": payload}), CLEAN_SDIST)
        return ["--config", str(config), "--dist", str(dist)]
    if name == "dirty_path":
        named = f"pkg/{_GIVEN.lower()}_notes.py"
        dist = make_dist(tmp, _with(CLEAN_WHEEL, **{named: CLEAN_TEXT.encode()}), CLEAN_SDIST)
        return ["--config", str(config), "--dist", str(dist)]
    ...
    if name == "nested_license":
        ...
        dist = make_dist(tmp, _with(CLEAN_WHEEL, **{nested: DIRTY_DOCSTRING.encode()}), CLEAN_SDIST)
        return ["--config", str(config), "--dist", str(dist)]
    if name == "backslash_member":
        dist = make_dist(
            ...
            CLEAN_WHEEL,
            CLEAN_SDIST,
            raw_sdist_member=("docs" + BACKSLASH + "x.md", DIRTY_PROSE.encode()),
        )
        return ["--config", str(config), "--dist", str(dist)]
    ...
    tree_files = {
        ...
        "segment_not_prefix": _with(CLEAN_TREE, **{"src/docs/x.py": DIRTY_PROSE.encode()}),
...
def _drop_trailing_wildcard(src: str) -> str:
    ...
    return src.replace(r'rf"\b{_GIVEN}\w*"', r'rf"\b{_GIVEN}\b"', 1)
...
def _egg_info_at_root_only(src: str) -> str:
    ...
    This is the measured defect itself, restored: `[^/]+` cannot span a
    separator, so a
...
def _floor_may_be_exempt(src: str) -> str:
    ...
    return src.replace("        if is_authorship(entry, config):", "        if False:", 1)


def _root_files_are_components(src: str) -> str:
    """Count a root-level file as a top-level component again."""
    return src.replace("        if sep and head:", "        if head:", 1)
...
def main(argv: list[str]) -> int:
    ...
    if survived or crashed:
        print(
            f"FAILED: {len(survived)} mutant(s) survived, {len(crashed)} only crashed",
            file=sys.stderr,
        )
    ...
    for label, narrowing, remove_detector in CONTROL_PAIRS:
        ...
        try:
            # Bound as defaults rather than closed over: the loop variables
            # would otherwise be looked up when the lambda RUNS, which is the
            # late-binding shape a linter refuses on sight even where, as
            # here, the call happens inside the same iteration.
            both = mutate(
                src,
                lambda s, _strip=remove_detector, _narrow=narrowing: _strip(_narrow(s)),
                tmp,
            )
            if denied_by(both) is not None:
                print(
                    f"FAILED control: with {label}, the narrowing it hides was "
                    "still caught. That is not a pass. Something other than "
                    "the detector is refusing it, so the evidence that the "
                    "detector is load bearing no longer exists. Re-read the "
                    "pair before changing it.",
                    file=sys.stderr,
                )
                return 1
        finally:
    ...
    print(
        ...
        "by its own detector and passes once that detector is removed"
    )
    return 0
