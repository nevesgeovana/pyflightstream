...
produces. A needle on the report's unconditional wording is what this repository
has twice been bitten by, so the needle sits on the violation's own words.

THE TWO CASES THAT MATTER are the two directions the original defect showed
in a single commit, and they pull in opposite directions, which is why a
window-based guard could not satisfy both at any width:
...
CASES: list[tuple[str, dict[str, str], int, str]] = [
    ("neighbour_env_does_not_excuse", {"test_a.py": NEIGHBOUR}, 1, "passes no env="),
    ("env_far_from_its_own_opening_line", {"test_a.py": FAR_KEYWORD}, 0, "0 unguarded"),
    (
        "bare_run_imported_from_subprocess_is_a_spawn",
        {"test_a.py": BARE_IMPORTED},
        1,
        "passes no env=",
    ),
    (
        "bare_run_that_is_somebody_elses_function_is_not",
        {"test_a.py": BARE_NOT_IMPORTED},
        0,
        "0 spawn call(s)",
    ),
    ("an_aliased_import_is_still_a_spawn", {"test_a.py": ALIASED}, 1, "sp.Popen passes no env="),
    (
        "check_output_is_a_spawn_too",
        {"test_a.py": OTHER_FUNCTIONS},
        1,
        "subprocess.check_output passes no env=",
    ),
    ("a_kwargs_splat_is_unverifiable_not_an_offender", {"test_a.py": SPLAT}, 0, "UNVERIFIABLE"),
    ("a_clean_module_passes", {"test_a.py": CLEAN}, 0, "0 unguarded"),
    (
        "every_module_is_walked_not_only_test_prefixed",
        {"test_a.py": CLEAN, "hook_entry.py": HELPER},
        1,
        "passes no env=",
    ),
    (
        "a_module_that_does_not_parse_is_refused",
        {"test_a.py": CLEAN, "broken.py": BROKEN},
        2,
        "does not parse",
    ),
    ("an_empty_directory_is_a_config_error", {}, 2, "An audit that examined nothing"),
]
...
MUTANTS: list[tuple[str, str, str, str]] = [
    ...
    (
        "judge a call by every keyword in its module, which is what a line window approximates",
    ...
    (
        "a **kwargs splat is an offender, which is the false red the other direction",
        "        if None in keywords:",
        "        if False:",
...
def run_case(module: Path, files: dict[str, str]) -> tuple[int, str]:
    with tempfile.TemporaryDirectory(prefix="spawn-env-") as tmp:
        ...
        r = subprocess.run(
            [sys.executable, str(module), str(target)], capture_output=True, text=True
        )
...
def main() -> int:
    if not MODULE.is_file():
        ...
        return 2
    print(f"check_spawn_env guard evidence, {len(CASES)} cases, {len(MUTANTS)} mutants")
    failed: list[str] = []
    for name, files, code, needle in CASES:
        got, out = run_case(MODULE, files)
        ok = got == code and needle in out
        print(
            f"  [{'ok ' if ok else 'FAIL'}] {name}: exit {got} "
    ...
    with tempfile.TemporaryDirectory(prefix="spawn-env-mutants-") as tmp:
        for i, (label, old, new, case_name) in enumerate(MUTANTS):
            if source.count(old) != 1:
                print(
                    f"  [FAIL] mutant {i} ({label}): the text it replaces "
            ...
            files, code, needle = next((f, c, n) for nm, f, c, n in CASES if nm == case_name)
            ...
            print(
                f"  [{'denied ' if denied else 'SURVIVED'}] {label} "
    ...
    if crash_denials:
        print(
            f"{len(crash_denials)} of them were denied BY A CRASH rather "
