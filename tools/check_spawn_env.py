...
call, and one commit met both of its failure directions at once:
...
that passes silently over what it could not read is the failure this
repository registers most often.
...
def main(argv: list[str]) -> int:
    ...
    if not roots:
        print("usage: check_spawn_env.py <directory> [<directory> ...]", file=sys.stderr)
        return 2
    try:
    ...
    if modules == 0:
        ...
        print(
            f"CONFIG: no *.py module under {', '.join(str(r) for r in roots)}"
    ...
    print(
        f"checked {modules} module(s), {spawns} spawn call(s), "
